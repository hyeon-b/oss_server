...
const hpApplyRouter = (router)=>{
    ...
    router.get('/helper/apply/:hp_id', this.HpApplyController.getHpApply);

    // 헬퍼 지원 완료하기 -> 지원한 신청서의 시간까지 post
    router.post('/helper/apply',this.HpApplyController.postHpApply);

    // 헬퍼 마이페이지) 기존 자기소개서 가져오기
    router.get('/helper/preidc/:hp_id',this.HpApplyController.getHpPreIdc);

    // 헬퍼 마이페이지) 기존 자기소개서 insert or update
    router.post('/helper/preidc',this.HpApplyController.postHpPreIdc);
